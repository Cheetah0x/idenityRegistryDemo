...
export async function getVkeysFromFixture() {
  ...
  const vkeys = {
    vkey_a: ensureVkeySize(vkeyFields.vkey_a.map((f: string) => BigInt(f.startsWith('0x') ? f : '0x' + f)), 128),
    vkey_b: ensureVkeySize(vkeyFields.vkey_b.map((f: string) => BigInt(f.startsWith('0x') ? f : '0x' + f)), 128),
    vkey_c: ensureVkeySize(vkeyFields.vkey_c.map((f: string) => BigInt(f.startsWith('0x') ? f : '0x' + f)), 128),
    vkey_d: ensureVkeySize(vkeyFields.vkey_d.map((f: string) => BigInt(f.startsWith('0x') ? f : '0x' + f)), 128)
  }
...
/**
 * Helper function to ensure verification key has exactly the expected size
 */
function ensureVkeySize(arr: bigint[], expectedSize: number): bigint[] {
  return ZKPassportHelper.ensureSize(arr, expectedSize);
}
...
export async function getProofsFromFixture() {
  ...
  const fieldsA = proofToFields(proofData.proof_a.proof);
  const fieldsB = proofToFields(proofData.proof_b.proof);
  const fieldsC = proofToFields(proofData.proof_c.proof);
  const fieldsD = proofToFields(proofData.proof_d.proof);
  ...
  const proofs = {
    proof_a: ZKPassportHelper.ensureSize(fieldsA.map((f: string) => BigInt(f.startsWith('0x') ? f : '0x' + f)), 459),
    proof_b: ZKPassportHelper.ensureSize(fieldsB.map((f: string) => BigInt(f.startsWith('0x') ? f : '0x' + f)), 459),
    proof_c: ZKPassportHelper.ensureSize(fieldsC.map((f: string) => BigInt(f.startsWith('0x') ? f : '0x' + f)), 459),
    proof_d: ZKPassportHelper.ensureSize(fieldsD.map((f: string) => BigInt(f.startsWith('0x') ? f : '0x' + f)), 459)
  }
